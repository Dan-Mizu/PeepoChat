...
const filterEmojis = () => {
	...
	Object.keys(emojis).forEach((key) => {
		...
		if (key === props.activeTab) {
			(emojis as IEmojiGroups)[key as EmojiGroupNames].forEach(
				(emoji) => {
					// if search key match
					if (
						emoji["n"][0].includes(
							props.keyword.toLocaleLowerCase()
						)
					) {
						let result = emoji.u;
						// check skin tone
						if (
							store.emojiSkinTone !== "neutral" &&
							Array.isArray(emoji.v)
						) {
							const v_index =
								emoji.v?.findIndex((v) =>
